...
Symbol.bindElementAction(compId, symbolName, "${_accept_blue_payment}", "click", function(sym,e){
	sym.play(7000);
	var ros=sym.getVariable("ROS");
	var UserOrderClient=new ROSLIB.ActionClient(
		{
			ros:ros,
			serverName:sym.getVariable("send_order"),
	...
	for(var i in ordered_coffee_list){
		...
		if(foundDuplicatedMenu==false){
			var menu=new ROSLIB.Message({name:ordered_coffee_list[i],size:1,qty:1});
			menus.push(menu);
		}
	}
	for(var i in ordered_sandwich_list){
		...
		for(var j in menus){
			...
			{
				menus[j].qty++;foundDuplicatedMenu=true;
				break;
			}
		}
		if(foundDuplicatedMenu==false)
		{
			var menu=new ROSLIB.Message({name:ordered_sandwich_list[i],size:1,qty:1});
			menus.push(menu);
		}
	}
	...
	goal.on('feedback', function(feedback) {
		...
		else console.log(feedback.status);
	});
	
	
	goal.on('result', function(result) { console.log(result); });
	...
	//Edge binding end
      ...
      Symbol.bindElementAction(compId, symbolName, "${_ROCON_BANK_CARD}", "click", function(sym, e) {
      ...
      Symbol.bindElementAction(compId, symbolName, "${_YUJINMASTER}", "click", function(sym, e) {
         // insert code for mouse click here
         sym.play(4100);
      ...
      Symbol.bindElementAction(compId, symbolName, "${_ROCON_BANK_DESC}", "click", function(sym, e) {
         // insert code for mouse click here
         sym.stop(4000);
      ...
      Symbol.bindElementAction(compId, symbolName, "${_YUJINMASTER_DESC}", "click", function(sym, e) {
         // insert code for mouse click here
         sym.play(4100);
