...
yepnope({nope:['include/EventEmitter2/eventemitter2.js'],complete:emitterLoaded});function emitterLoaded()
...
//Edge binding end
Symbol.bindElementAction(compId,symbolName,"${_accept_blue}","click",function(sym,e){sym.play(3000);});
//Edge binding end
...
//Edge binding end
Symbol.bindTriggerAction(compId,symbolName,"Default Timeline",7948,function(sym,e){sym.play(7000);});
//Edge binding end
Symbol.bindTriggerAction(compId,symbolName,"Default Timeline",8950,function(sym,e){sym.play(8000);});
//Edge binding end
Symbol.bindTriggerAction(compId,symbolName,"Default Timeline",9950,function(sym,e){sym.play(9000);});
//Edge binding end
Symbol.bindTriggerAction(compId,symbolName,"Default Timeline",2904,function(sym,e){sym.stop();});
//Edge binding end
Symbol.bindElementAction(compId,symbolName,"${_accept_blue_payment}","click",function(sym,e){sym.play(7000);var ros=sym.getVariable("ROS");var UserOrderClient=new ROSLIB.ActionClient({ros:ros,serverName:sym.getVariable("send_order"),actionName:'cafe_msgs/UserOrderAction'});var tableID=sym.getVariable("tableID");var tableID_num=parseInt(tableID);var menus=new Array();var ordered_coffee_list=sym.getVariable("ordered_coffee_list");var ordered_sandwich_list=sym.getVariable("ordered_sandwich_list");for(var i in ordered_coffee_list){var foundDuplicatedMenu=false;for(var j in menus){if(ordered_coffee_list[i]==menus[j].name){menus[j].qty++;foundDuplicatedMenu=true;break;}}
if(foundDuplicatedMenu==false){var menu=new ROSLIB.Message({name:ordered_coffee_list[i],size:1,qty:1});menus.push(menu);}}
for(var i in ordered_sandwich_list){var foundDuplicatedMenu=false;for(var j in menus){if(ordered_sandwich_list[i]==menus[j].name)
{menus[j].qty++;foundDuplicatedMenu=true;break;}}
if(foundDuplicatedMenu==false)
{var menu=new ROSLIB.Message({name:ordered_sandwich_list[i],size:1,qty:1});menus.push(menu);}}
...
sym.$("status_text").html("Waiterbot is going to kitchen.");else if(feedback.status==2){sym.$("status_text").html("Waiterbot is arrived at kitchen.");sym.play(8000);}
else if(feedback.status==3)
sym.$("status_text").html("Waiterbot is waiting for foods at kitchen.");else if(feedback.status==4){sym.$("status_text").html("Waiterbot is in delivery.");sym.play(9000);}
else if(feedback.status==5){sym.$("status_text").html("Waiterbot is arrived at the table.");}
else if(feedback.status==6){sym.$("status_text").html("Waiterbot is waiting for user confirmation.");sym.play(10000);}
else if(feedback.status==7){sym.$("status_text").html("Delivery service is completed.");sym.play(0);}
else console.log(feedback.status);});goal.on('result',function(result){console.log(result);});console.log("goal.send()");goal.send();sym.$("status_text").html("Order is received.");});
//Edge binding end
Symbol.bindElementAction(compId,symbolName,"${_deleteCopy2}","click",function(sym,e){sym.play(0000);});
//Edge binding end
...
//Edge binding end
Symbol.bindElementAction(compId,symbolName,"${_ROCON_BANK_CARD}","click",function(sym,e){sym.stop(4000);});
//Edge binding end
Symbol.bindElementAction(compId,symbolName,"${_YUJINMASTER}","click",function(sym,e){sym.play(4100);});
//Edge binding end
Symbol.bindElementAction(compId,symbolName,"${_ROCON_BANK_DESC}","click",function(sym,e){sym.stop(4000);});
//Edge binding end
Symbol.bindElementAction(compId,symbolName,"${_YUJINMASTER_DESC}","click",function(sym,e){sym.play(4100);});
//Edge binding end
})("stage");
